...
export default function SectionDivider({ id, title, subtitle, accentColor = "#a78bfa" }: SectionDividerProps) {
    return (
        <section
            ...
            className={`relative py-24 md:py-32 overflow-hidden scene-landing-divider ${id === "vr" ? "scene-landing-vr" : ""}`}
            style={{ backgroundColor: "oklch(0.97 0.015 85)" }} // Cream background
        >
            {id === "vr" && <div className="scene-landing-signal" aria-hidden="true"><span />继续探索<span /></div>}
