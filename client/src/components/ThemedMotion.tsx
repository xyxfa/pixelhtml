  ...
  return (
    <>
      <span className={`theme-detail theme-detail-${detail}`}>
        ...
        <i />
        <i />
        {(detail === "path" || detail === "circuit" || detail === "post") && (
          <svg viewBox="0 0 160 100" fill="none">
            <path
              d={
                detail === "circuit"
                  ? "M10 80H52V48H108V20H150"
                  : "M10 82C10 30 78 90 80 48S140 0 148 28"
              }
            />
          </svg>
        )}
      ...
      <img
        className="theme-main-sprite"
        src={`/themed-motion/${sprite}.webp`}
        alt=""
        draggable={false}
        decoding="async"
      />
...
export function ThemeEmblem({ theme }: { theme: MotionTheme }) {
  ...
  return (
    <span
      className={`theme-emblem theme-${theme}`}
      data-running={running}
      aria-hidden="true"
      style={{ "--theme-accent": motionThemes[theme].accent } as CSSProperties}
    >
      <Motif theme={theme} />
    </span>
  );
...
export default function ThemedMotion({ theme }: { theme: MotionTheme }) {
  ...
  return (
    <div
      ref={ref}
      className={`themed-motion theme-${theme}`}
      data-theme={theme}
      data-running={running && visible}
      aria-hidden="true"
      style={{ "--theme-accent": accent } as CSSProperties}
    >
      <div className="theme-corner">
        <Motif theme={theme} />
      </div>
      {(gameSceneThemes as readonly string[]).includes(theme) && (
        <GameMotionScene theme={theme as (typeof gameSceneThemes)[number]} />
      )}
      {!reduced && (
        <>
          <span
            className="theme-orbiting-sprite"
            style={motionVariation(seed, 0)}
          >
            <img
              src={`/themed-motion/${sprite}.webp`}
              alt=""
          ...
          <span className="theme-traveler" style={motionVariation(seed, 1)}>
            <img
              src={`/themed-motion/${sprite}.webp`}
              alt=""
              draggable={false}
          ...
          <span
            className="theme-edge theme-edge-a"
            style={{
              ...motionVariation(seed, 2),
              top: `${24 + motionSample(seed, 2, 11) * 18}%`,
            }}
          >
            <img
              src={`/themed-motion/${sprite}.webp`}
          ...
          <span
            className="theme-edge theme-edge-b"
            style={{
              ...motionVariation(seed, 3),
              top: `${55 + motionSample(seed, 3, 11) * 20}%`,
            }}
          >
            <img
              src={`/themed-motion/${sprite}.webp`}
          ...
          <span
            className="theme-edge theme-edge-c"
            style={{
              ...motionVariation(seed, 4),
              top: `${78 + motionSample(seed, 4, 11) * 14}%`,
            }}
          >
            <img
          ...
          <span className="theme-drift-dots">
            <i />
            <i />
            <i />
          </span>
