        ...
        <div className="grid lg:grid-cols-5 gap-8 items-start max-w-5xl mx-auto">
          ...
          <div className="lg:col-span-2">
            <div className="wood-panel p-6 text-center">
              <div
                className="w-40 h-40 mx-auto mb-4 border-4 border-wood rounded-lg overflow-hidden"
              ...
              <h3 className="font-pixel text-xs text-wood-dark mb-1">
                YOUR NAME
              </h3>
              <p className="font-pixel text-[7px] text-leaf mb-4">
                GAME DEVELOPER
              </p>

              <div className="border-t-2 border-wood/20 pt-4 mt-4">
                <p className="font-pixel text-[7px] text-wood-light mb-3">
                  裝備欄
                </p>
                <div className="flex flex-wrap gap-2 justify-center">
                  {tools.map(tool => (
                    <span key={tool} className="pixel-tag">
                      {tool}
                    </span>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mt-6 pt-4 border-t-2 border-wood/20">
                {stats.map(s => (
                  <div key={s.label} className="text-center">
                    <div className="font-pixel text-sm text-coral">
                      {s.value}
                    </div>
                    <div className="font-pixel text-[6px] text-wood-light mt-1">
                      {s.label}
                    </div>
                  </div>
                ))}
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="wood-panel p-6">
              <h4 className="font-pixel text-[9px] text-leaf mb-4">
                {t("about.title")}
              </h4>
            ...
            <div className="wood-panel p-6">
              <h4 className="font-pixel text-[9px] text-leaf mb-5">
                {t("about.skills")}
              </h4>
                ...
                {skills.map(skill => (
                  <div key={skill.name}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="font-pixel text-[7px] text-wood-dark">
                        {skill.name}
                      </span>
