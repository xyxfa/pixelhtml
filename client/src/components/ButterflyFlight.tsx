...
export default function ButterflyFlight({
  ...
  useEffect(() => {
    ...
    const resize = new ResizeObserver(entries => {
      scale = entries[0].contentRect.width / 1024;
    });
    ...
    const tick = (time: number) => {
      ...
      if (flight.rest > 0) {
        ...
        if (flight.elapsed >= flight.duration) {
          ...
          flight.to = {
            x: random(-64, 72) * scale,
            y: random(-38, 30) * scale,
          };
          flight.c1 = {
            x: flight.from.x + Math.cos(flight.heading) * 25 * scale,
            y: flight.from.y + Math.sin(flight.heading) * 20 * scale,
          };
          flight.c2 = {
            x: flight.to.x + random(-28, 28) * scale,
            y: flight.to.y + random(-25, 25) * scale,
          };
          flight.duration = random(2200, 4700);
          flight.elapsed = 0;
          facing.style.setProperty("--wing-speed", `${random(0.32, 0.52)}s`);
        }
        ...
        if (flight.elapsed >= flight.duration) {
          ...
          facing.style.setProperty("--wing-speed", `${random(0.7, 1)}s`);
        }
      }
  ...
  return (
    <span
      ref={ref}
      className="hero-mote hero-butterfly"
      data-facing="left"
      style={
        {
          left: `${x}%`,
          top: `${y}%`,
          width: `${width}%`,
          "--wing-phase": `${index * -0.137}s`,
        } as CSSProperties
      }
    >
      <span>
        <img
          src="/hero-atmosphere/butterfly-open.webp"
          alt=""
          draggable={false}
        />
        <img
          src="/hero-atmosphere/butterfly-folded.webp"
          alt=""
          draggable={false}
        />
      </span>
